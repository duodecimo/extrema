...
from .serializers import (AdminUserSerializer, EmailVerificationSerializer,
                          LoginSerializer, LogoutSerializer,
                          RegisterSerializer,
                          ResetPasswordEmailRequestSerializer,
                          SetNewPasswordSerializer, UserEmailCheckSerializer,
                          UserSerializer)
...
class RegisterView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        email_body = 'Olá, '+user.name + \
            ' , use o link abaixo para verificar seu email junto ao site EAT \n' + absurl
        data = {
    ...
    def get(self, request):
        ...
        try:
            ...
            return render(request, 'email_verify.html',
                          {'mensagem': 'Ativado com Successo!!!'})
        except jwt.ExpiredSignatureError as identifier:
            # return Response({'error': 'Activation Expired'},
            #                 status=status.HTTP_400_BAD_REQUEST)
            return render(request, 'email_verify.html',
                          {'mensagem': 'Ativação Expirada.'})
        except jwt.exceptions.DecodeError as identifier:
            if debugging == True:
                print(
                    'Em apps/user/views.py, VerifyEmail: DecodeError ...')
                print('Unverifyed header: ', jwt.get_unverified_header(token))
                print('claims without validation: ',
...
class AdminUserViewSet(generics.GenericAPIView):
    ...
    def patch(self, request):
        ...
        if request.user == "AnonymousUser":
            return Response('You are not logged',
                            status=status.HTTP_400_BAD_REQUEST)
            ...
            result = {
                ...
                "message":
                f"you are a superuser and {user.name} is admin now"
            }
